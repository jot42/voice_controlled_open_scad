...
"""
NOTE: make scrolling on textbox limited to line/column count or screen height/width (whichevers lower)
      ...
      when adding lines to the array, ensure that the last character is a space, if it isnt already
...
screen_main.addstr(1,0, "| VCOS-" + OS_IDENTIFIER + ' ' + VERSION + " | " + " Press 'h' or say 'help' for command help |")
...
def update_text_window(sh, sw, tc):
    ...
    # update_text_window(window_rows,window_cols - 2, text_box)
        ...
        for rows in range(container_rows - 1):
            ...
            screen_container_editor.addstr(rows, 0, temp_string[screen_scroll_h:container_cols])
        ...
        screen_container_editor.addstr(cursor_scroll_v , cursor_scroll_h, text_box[cursor_scroll_v + screen_scroll_v][cursor_scroll_h + screen_scroll_h] , curses.A_BLINK)
        ...
        if Exception == curses.error:
            print("FUCK")
        
        update_command_window(container_rows, "Error - 0x01")
        print("CSV: ", str(cursor_scroll_v))
        print("ROWS: ", str(container_rows))
        print("Loop Progress: ", str(www))
        traceback.print_exc()
        return False
...
def add_text(t):
    ...
    try:
        ...
        for i in range(len(t)):
            cursor_scroll_h += 1
            update_text_window(container_rows, container_cols, text_box)
...
def input_parser():
    ...
    # L Key - Load File
        ...
        for line in f:
            # Replace all newline and tabs in the line
            text_box.append(line.replace('\t', ' ').replace('\n',' '))
            
            while len(line) < container_cols:
                line = line + " "
        ...
        if update_text_window(container_rows, container_cols, text_box):
